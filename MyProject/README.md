...
* Check the `ConnectionStrings` in `appsettings.json` files under the `MyProject.HttpApi.Host` and `MyProject.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 5c134e3c-1842-4af0-8e2f-da1a00344840
```
...
Run `MyProject.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
* `MyProject.Blazor`: ASP.NET Core Blazor Server application that is the essential web application of the solution.
