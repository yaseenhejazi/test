...
* Check the `ConnectionStrings` in `appsettings.json` files under the `TestProject.HttpApi.Host` and `TestProject.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 8de1c82d-b062-4532-a680-2e2756195b7b
```
...
* `TestProject.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
* `TestProject.Blazor`: ASP.NET Core Blazor Server application that is the essential web application of the solution.
